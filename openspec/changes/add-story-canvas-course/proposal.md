...
## Decision Record（2026-08-01 修訂）

初版課綱直接沿用《一人公司的故事骨架卡》的五欄，主軸落在產出一句話定位。Vista 指出這與站上既有的定位收斂工作坊高度重疊，會產生排斥效應，要求改為強化故事帶動買氣與轉單的元素，並從既有著作取材。

修訂後的定位：定位收斂處理「你是誰」，本課程處理「你怎麼講才會有人掏錢」。課名改為《故事力就是你的成交力》，沿用 Vista 著作《文案力就是你的鈔能力》的句式。課綱改為四模組（挑出成交故事、四個賣點位置、有感改寫、雙層轉述測試），骨架卡五欄降為模組一的工作表。驗收判準升為兩層，第二層是聽者會不會想問價格。

Vista 另裁定：頁面不提另一堂課的名稱與講師，只靠對象與產出的差異自然區隔。
