...
export default function RegistrationTable({ registrations, eventId }: Props) {
  ...
  const exportCSV = () => {
    ...
    const rows = registrations.map((r) => [
      r.name, r.email, r.phone || "", r.ticket_types?.name || "", statusConfig[r.status]?.label || r.status,
      ...
      <CardContent className="p-4">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            {selected.length > 0 && (
              <>
                <Button size="sm" variant="outline" onClick={() => bulkUpdate("confirmed")} disabled={updating}>確認</Button>
                <Button size="sm" variant="outline" onClick={() => bulkUpdate("waitlisted")} disabled={updating}>候補</Button>
                <Button size="sm" variant="outline" onClick={() => bulkUpdate("cancelled")} disabled={updating}>取消</Button>
              </>
            )}
            <Button size="sm" variant="outline" onClick={exportCSV}>匯出 CSV</Button>
          </div>
        </div>
              ...
              {filtered.map((reg) => {
                ...
                return (
                  <tr key={reg.id} className="border-b hover:bg-muted/50">
                    ...
                    <td className="p-2 text-muted-foreground">{new Date(reg.created_at).toLocaleString("zh-TW")}</td>
                  </tr>
                );
              })}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-muted-foreground">沒有找到報名紀錄</td>
                </tr>
              )}
            </tbody>
          </table>
