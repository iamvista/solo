...
export default async function EventPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Google Calendar URL
  const calStart = startDate.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}/, "");
  ...
  const jsonLd = {
    ...
    organizer: event.organizer
      ? { "@type": "Person", name: event.organizer.display_name || "自由人學院" }
      ...
      <article className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        {isArchived && (
          <div className="mb-6 rounded-lg bg-muted p-4 text-center">
            <p className="text-muted-foreground">此活動已結束，以下為歷史紀錄。</p>
          </div>
        )}
        ...
        <div className="mb-8">
          <div className="mb-3 flex flex-wrap gap-2">
            ...
            {event.category && <Badge variant="secondary">{event.category === "workshop" ? "工作坊" : event.category === "lecture" ? "講座" : event.category === "meetup" ? "聚會" : "研討會"}</Badge>}
          ...
          <CardContent className="grid gap-4 p-6 sm:grid-cols-2">
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="font-medium">{event.format === "online" ? "線上活動" : event.venue_name || "待通知"}</p>
            ...
            <div className="space-y-3">
              {event.organizer && (
                <div>
                  <p className="text-sm font-medium text-muted-foreground">主辦人</p>
                  <p className="font-medium">{event.organizer.display_name}</p>
                </div>
              )}
              <div className="flex gap-3">
                <a href={calendarUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">加入 Google 日曆</a>
              </div>
        ...
        {event.youtube_embed && (
          <div className="mb-8 aspect-video w-full overflow-hidden rounded-xl">
            <iframe src={event.youtube_embed} className="h-full w-full" allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
          </div>
        )}
                ...
                <Card key={update.id}>
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">{new Date(update.created_at).toLocaleDateString("zh-TW")}</span>
                    </div>
                    {update.content && <p className="mt-2 text-sm text-muted-foreground whitespace-pre-wrap">{update.content}</p>}
                  </CardContent>
                </Card>
              ))}
            </div>
