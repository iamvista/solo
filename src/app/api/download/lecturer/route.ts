...
import { LECTURER_BLOB_PATHNAME, LECTURER_DOWNLOAD_FILENAME } from "@/lib/lecturer-kit";
...
export async function GET(request: NextRequest) {
  ...
  // 沿用 army／ai-coach-kit／ars 下載 route 的限流慣例：30 次/分鐘/IP。
  const ip = getClientIp(request.headers);
  if (!checkRateLimit(`download-lecturer:${ip}`, { max: 30, windowMs: 60_000 })) {
  ...
  // Entitlement 一律以 product_id 比對；非本系 token（如 army-kit、ai-coach-kit、ars）一律 403。
  if (tokenRecord.product_id !== "lecturer-kit") {
    return NextResponse.json(
      { error: "此下載連結不適用於本項目" },
      { status: 403 },
    );
  ...
  try {
    blob = await head(LECTURER_BLOB_PATHNAME);
    ...
    console.error("[download/lecturer] blob fetch failed", err);
    return NextResponse.json(
  ...
  // affected rows（RETURNING 是否有列）判斷放行，避免併發下載衝破上限（比照 download/army）。
  ...
  if (incrementError) {
    console.error(
      "[download/lecturer] increment_download_count failed",
      incrementError,
    );
    return NextResponse.json(
  ...
  return new NextResponse(blobResp.body, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${LECTURER_DOWNLOAD_FILENAME}"`,
