  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Badge variant="outline" className="mb-2">活動管理</Badge>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="mb-2 flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">{formatConfig[event.format] || event.format}</span>
                        {event.is_featured && <Badge variant="outline" className="border-yellow-400 bg-yellow-50 text-yellow-700">推薦</Badge>}
                      </div>
                      <CardTitle className="text-lg">
                        <Link href={`/admin/events/${event.id}/edit`} className="hover:underline">
                          {event.title}
                        </Link>
                      </CardTitle>
                    </div>
                    <div className="flex shrink-0 gap-2">
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/admin/events/${event.id}/edit`}>編輯</Link>
                      </Button>
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/admin/events/${event.id}/registrations`}>報名</Link>
                      </Button>
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/admin/events/${event.id}/updates`}>公告</Link>
                      </Button>
                    </div>
                  </div>
                </CardHeader>
